...
14;explanation;Vehicle requests are subject to availability.;false
...
23;text;Notes and special requests;false
FIELDS
    attrs = row.to_hash.merge form_id: form.id
    Field.create! attrs
  end
  # Couldn't make options work with CSV, so create manually
  Field.create! form_id: form.id, number: 13, data_type: 'options', prompt: 'Vehicle requested', required: true, options: ['Sedan', 'Van']
end

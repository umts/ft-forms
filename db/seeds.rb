...
# USERS
FactoryBot.create :user, first_name: 'David',
                         last_name: 'Faulkenberry',
                         staff: true,
                         email: 'dfaulken@umass.edu',
...
ActiveRecord::Base.transaction do
  ...
  Field.create! form_id: form.id, number: 13, data_type: 'options',
                prompt: 'Desired vehicle type', required: true, options: %w[Sedan Van]
  Field.create! form_id: form.id, number: 23, data_type: 'options',
                prompt: 'Desired vehicle type', required: false, options: %w[Sedan Van]
end
